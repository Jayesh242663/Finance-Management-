...
const StudentForm = () => {
  ...
  // Build course options from Supabase so all active courses appear in add/edit flows.
  const displayCourses = useMemo(() => {
    const sourceCourses = (courses && courses.length > 0)
      ? courses.map((course) => ({
          value: course.course_type || course.course_code || course.course_name,
          label: course.course_name || course.course_code || course.course_type,
        }))
      : COURSES;

    const unique = sourceCourses.filter((course, index, arr) =>
      course.value && arr.findIndex((item) => item.value === course.value) === index
    );

    unique.sort((a, b) => a.label.localeCompare(b.label));

    if (watchedCourse && !unique.some((course) => course.value === watchedCourse)) {
      unique.push({ value: watchedCourse, label: watchedCourse });
    }

    return unique;
  }, [courses, watchedCourse]);
