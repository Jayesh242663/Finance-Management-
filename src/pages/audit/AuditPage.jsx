...
const AuditPage = () => {
  ...
  const filteredAuditLog = useMemo(() => {
    ...
    const financialLogs = allLogs.filter(entry => (
      ...
      (entry.action === 'UPDATE' && entry.entityType === 'PLACEMENT' && entry.details?.installmentId) ||
  ...
  const ledgerData = useMemo(() => {
    ...
    const entries = sortedLog.map((entry, index) => {
      const isPlacementPaymentUpdate = entry.action === 'UPDATE' && entry.entityType === 'PLACEMENT' && entry.details?.installmentId;
      ...
      if (isPlacementPaymentUpdate) {
        // Amendment rows are informational only; they should not affect the ledger balance.
      } else if (isCredit) {
        totalCredits += amount;
  ...
  const getTransactionNarration = (entry) => {
    ...
    switch (entry.action) {
      ...
      case 'CREATE':
        ...
        return `Being new student admission - ${course} programme (${entry.details?.batch || '2025-26'})`;
      case 'UPDATE':
        if (entry.entityType === 'PLACEMENT' && entry.details?.installmentId) {
          ...
          const changeEntries = entry.details?.changes || {};
          const changeSummary = Object.entries(changeEntries)
            .map(([field, change]) => {
              const label = {
                amount: 'amount',
                paymentDate: 'payment date',
                paymentMethod: 'payment method',
                bankMoneyReceived: 'bank account',
                chequeNumber: 'cheque number',
                remarks: 'remarks',
              }[field] || field;

              if (!change || (change.before === change.after)) return null;
              return `${label}: ${formatValue(change.before, field)} → ${formatValue(change.after, field)}`;
            })
            .filter(Boolean)
            .join('; ');

          const fields = changeSummary || entry.details?.updatedFields?.join(', ') || 'placement payment details';
          return `Being placement payment amendment - ${fields}`;
        }
